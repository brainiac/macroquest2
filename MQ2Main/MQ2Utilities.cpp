...
VOID DebugSpew(PCHAR szFormat, ...)
{
	...
	int len = _vscprintf(szFormat, vaList) + 1 + 32;// _vscprintf doesn't count // terminating '\0'
	if (char *szOutput = (char *)LocalAlloc(LPTR, len)) {
		strcpy_s(szOutput, len, DebugHeader " ");
		vsprintf_s(szOutput + strlen(DebugHeader) + 1, len, szFormat, vaList);
		strcat_s(szOutput, len, "\n");
